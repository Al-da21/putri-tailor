        ...
        <div class="flex flex-col sm:flex-row gap-4">
            ...
            <div class="w-full">
                <div class="p-5 border border-indigo-100 bg-white shadow-md shadow-indigo-100 rounded-md">
                    ...
                    <div class="my-4 font-semibold">Jadwal Pengukuran</div>
                    <div class="p-5 border border-indigo-100 bg-white shadow-md shadow-indigo-100 rounded-md mb-6">
                        @if($transaction->jadwal_pengukuran_id)
                            <p>
                                Tanggal: {{
                                    \Carbon\Carbon::parse(
                                        \Carbon\Carbon::parse($transaction->jadwalPengukuran->tanggal_pengukuran)->format('Y-m-d') . ' ' .
                                        \Carbon\Carbon::parse($transaction->jadwalPengukuran->waktu_pengukuran)->format('H:i:s')
                                    )->format('d M Y')
                                }},
                                Jam: {{
                                    \Carbon\Carbon::parse(
                        ...
                        @else
                            <p>Belum ada jadwal pengukuran.</p>
                        @endif
                    </div>
                    ...
                    <div class="flex border-b-2 border-indigo-50 py-5">
                        ...
                        <h3 class="font-semibold text-center text-slate-600 text-xs uppercase w-1/5 text-center">Qty</h3>
                                ...
                                <div class="flex w-2/5 gap-3 pl-0 sm:pl-6">
                                    <div class="w-fit">
                                        <div class="w-16 md:w-24 h-16 md:h-24 rounded-md overflow-hidden">
                                            @if(isset($item_data['order_type']) && $item_data['order_type'] === 'CUSTOM' && isset($item_data['customOrder']))
                                                {{-- Gambar produk custom --}}
                                                <img class="w-full h-full object-cover"
                                                    src="{{ Storage::url($item_data['customOrder']->custom_image) }}"
                                    ...
                                    <div class="flex flex-col justify-center w-full">
                                        @if(isset($item_data['order_type']) && $item_data['order_type'] === 'CUSTOM' && isset($item_data['customOrder']))
                                            {{-- Nama produk custom --}}
                                            <span class="font-bold text-sm line-clamp-2">
                                                {{ $item_data['customOrder']->custom_name }}
                                            </span>
                                        @else
                                            {{-- Nama produk standar --}}
