            ...
            <div class="flex my-10">
                <div class="w-full sm:w-3/4 bg-white px-2 sm:px-10 py-10">
                    ...
                    <form action="{{ url('/cart/checkout') }}" method="POST">
                        @csrf
                        {{-- Bagian ini hanya untuk konten produk di dalam @foreach --}}
                        @foreach ($cart as $item)
                            <div class="flex items-center py-5 px-2 bg-white border border-indigo-200 mb-3 shadow-md shadow-indigo-100">
                                <div class="flex w-2/5 gap-3 sm:pl-4">
                                        ...
                                        <div class="w-16 md:w-24 h-16 md:h-24 rounded-md overflow-hidden">
                                            @if ($item->custom_image)
                                                <img class="w-full h-full object-cover"
                                                    src="{{ Storage::url($item->custom_image) }}" alt="Gambar Custom">
                                            @else
                                                <img class="w-full h-full object-cover"
                                                    src="{{ Storage::url($item->product->image) }}" alt="Gambar Produk">
                                            @endif
                                        </div>
                                    </div>
                                    <div class="flex flex-col gap-1 w-full">
                                        ...
                                        @if ($item->custom_material)
                                            <span class="text-xs text-slate-500">Bahan: {{ $item->custom_material }}</span>
                                        @endif

                                        {{-- @if ($item->custom_image)
                                            <div class="mt-1">
                                                <span class="text-xs text-slate-500 block">Referensi:</span>
                                                <img src="{{ Storage::url($item->custom_image) }}"
                                                    class="w-16 h-16 object-cover rounded-md border" alt="Custom Image">
                                            </div>
                                        @endif --}}
