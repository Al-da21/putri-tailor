...
@section('content')
    <div class="container mx-auto px-4">
        <div class="flex flex-col pt-24 justify-center items-center gap-4">
            ...
            <div>
                <div class="text-xl text-center font-semibold text-slate-800">Silahkan Transfer ke <span class="font-bold text-indigo-800">{{ $adminData->account_number }}</span> sebesar <span class="font-bold text-indigo-800">Rp {{ number_format($data->total, 0, ',', '.') }}</span></div>
                <div class="text-base text-center text-slate-500">Agar orderanmu dapat segera di proses</div>
            </div>
            <a href="{{ url('/') }}"
